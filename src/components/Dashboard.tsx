      ...
      <div className="absolute top-4 right-4 md:top-6 md:right-6 landscape:top-2 landscape:right-2 landscape:md:top-6 landscape:md:right-6 z-50">
        {adminPassword ? (
          <button 
            ...
            className="flex items-center space-x-2 bg-white/20 hover:bg-white/30 backdrop-blur-md px-3 md:px-4 py-1.5 md:py-2 landscape:px-2 landscape:py-1 landscape:md:px-4 landscape:md:py-2 rounded-full text-white text-[11px] md:text-sm font-bold transition-all border border-white/20"
          >
          ...
          <button 
            onClick={handleLogin}
            className="flex items-center space-x-2 bg-white/10 hover:bg-white/20 backdrop-blur-md px-3 md:px-4 py-1.5 md:py-2 landscape:px-2 landscape:py-1 landscape:md:px-4 landscape:md:py-2 rounded-full text-white/70 hover:text-white text-[11px] md:text-sm font-bold transition-all border border-white/10"
          >
      ...
      <div className="max-w-3xl w-full bg-white/90 backdrop-blur-md rounded-3xl md:rounded-[2.5rem] shadow-2xl p-5 md:p-12 landscape:p-4 landscape:md:p-12 flex flex-col items-center text-center space-y-8 md:space-y-10 landscape:space-y-4 landscape:md:space-y-10 border-4 border-white/20">
        ...
        <div className="space-y-1 md:space-y-2 landscape:space-y-0 landscape:md:space-y-2">
          <div className="flex items-center justify-center space-x-2 md:space-x-3 mb-2 md:mb-4 landscape:mb-1 landscape:md:mb-4">
            <Coffee className="w-8 h-8 md:w-10 md:h-10 landscape:w-6 landscape:h-6 landscape:md:w-10 landscape:md:h-10 text-slate-800" />
            <h1 className="text-3xl md:text-5xl landscape:text-2xl landscape:md:text-5xl font-black tracking-tight text-slate-900 uppercase">
              Coffee Tracker
            </h1>
          </div>
        ...
        {/* Main Timer Display */}
        <div className="w-full py-8 md:py-12 landscape:py-4 landscape:md:py-12 px-3 md:px-6 rounded-[2rem] md:rounded-3xl bg-slate-900 text-white shadow-inner relative overflow-hidden">
          <div className="relative z-10 flex flex-col items-center">
            <span className="text-[10px] md:text-sm landscape:text-[9px] landscape:md:text-sm font-bold text-slate-400 uppercase tracking-[0.3em] mb-2 md:mb-4 landscape:mb-1 landscape:md:mb-4 flex items-center">
              <Clock className="w-3.5 h-3.5 md:w-4 md:h-4 mr-1.5 md:mr-2" />
              {labelText}
            </span>
            
            {isReset || lastBrew === null ? (
              <div className="text-6xl md:text-9xl landscape:text-5xl landscape:md:text-9xl font-black tabular-nums tracking-tighter text-slate-300">
                --:--
              </div>
            ) : (
          ...
          <div className="absolute top-0 right-0 -mr-16 -mt-16 w-48 h-48 md:w-64 md:h-64 landscape:w-32 landscape:h-32 landscape:md:w-64 landscape:md:h-64 bg-white/5 rounded-full blur-3xl"></div>
        </div>
